...
def test_list_calls_correct_endpoint(mock_client_class):
    ...
    profiles = [
        {'id': PROFILE_ID, 'name': 'deterministic', 'model_id': MODEL_ID}
    ]
...
def test_create_sends_payload(mock_client_class):
    ...
    created = {'id': PROFILE_ID, 'name': 'creative', 'model_id': MODEL_ID, 'param_overrides': {}}
    mock_client_class.return_value = _mock_client('post', return_value=created, status_code=201)
    ...
    result = runner.invoke(
        ...
        [
            ...
            '--name', 'creative',
            '--model-id', MODEL_ID,
            '--param-overrides', '{"temperature": 1.0}',
        ],
    )
